...
def create_data_set(model_idx):
    ...
    word_embed_path = data_dir + 'yelp-embedding-200d.txt'
    usr_pdr_embed_path = data_dir + 'yelp.pkl'
    data_path = [data_dir + 'yelp_train.ss', data_dir + 'yelp_validate.ss',
                 data_dir + 'yelp_test.ss']
    # model_idx = 428

    viz_data_path = ['viz_data/yelp14_train_viz_data_{}.pkl'.format(model_idx),
                     'viz_data/yelp14_dev_viz_data_{}.pkl'.format(model_idx),
                     'viz_data/yelp14_test_viz_data_{}.pkl'.format(model_idx)]
...
def find_all_pairs(model_idx):
    obj_list = load('viz_log/yelp14_all_{}.pkl'.format(model_idx))
    print(len(obj_list))
    all_pairs = []
    # by pairs {a: , b:}
...
def yelp14_all_sample_terms(model_idx):
    ...
    pos_keys = [1, 568, 901, 1289, 1973, 2177, 2822, 3375, 3760, 4070, 4137, 4179, 4506, 4723, 4726] # len(pos_terms) < 50 and len(v) > 200
    all_pos_terms = []
    all_neg_terms = []
    for key in pos_keys:
...
def find_yelp14_all_sample_terms_data(model_idx):
    ...
    # viz_data_path = ['viz_data/yelp13/yelp13_train_viz_data_{}.pkl'.format(model_idx),
    #                  'viz_data/yelp13/yelp13_dev_viz_data_{}.pkl'.format(model_idx),
    #                  'viz_data/yelp13/yelp13_test_viz_data_{}.pkl'.format(model_idx)]
...
def generate_tex(model_idx):
    yelp14_all = load('viz_log/yelp14_all_{}.pkl'.format(model_idx))
    yelp14_all_sample_terms = load('viz_log/yelp14_all_{}_sample_terms.pkl'.format(model_idx)) # HY - Changed from all_sample_terms
    # pos_keys = [568, 901, 1289, 1973, 2177, 2822, 3375, 3760, 4070, 4137, 4179, 4506, 4723, 4726]
    pos_keys = yelp14_all_sample_terms['pos_keys']
    # # print(collections.Counter(yelp14_all_sample_terms['all_pos_terms']))
    # # pos_terms = list(collections.Counter(yelp14_all_sample_terms['all_pos_terms']).keys())
                ...
                if inst['uid'] == key:
                    ...
                    if rating in [0, 1, 2, 3, 4]:
                        ...
                        generate(list_word_attention_tuple,
                                 'plot/tex/{}/u{}_p{}_r{}.tex'.format(model_idx, key, pid, rating))
                        # print('-' * 10)
                        lines.append('-' * 10)
            # print('*' * 100)
            # lines.append('*' * 100)
