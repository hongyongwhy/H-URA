...
class DynamicRNN(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers=1, bias=True, batch_first=True, dropout=0.5, bidirectional=False, rnn_type='LSTM', use_cuda=False):
    ...
    def forward(self, input_data, hidden_in=None, lengths=None, flag_ranked=True):
        '''
        ...
        if flag_ranked is True:
            lengths_ranked, input_ranked = torch.LongTensor(lengths), input_data
        else:
            ...
            input_ranked = torch.index_select(input_data, dim=0, index=idx_select)
